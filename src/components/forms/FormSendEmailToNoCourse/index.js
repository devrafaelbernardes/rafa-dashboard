...
import objectMutation, { SEND_EMAIL_TO_NO_COURSE } from 'services/api/mutation';
import TextEditor from 'components/TextEditor';

export function FormSendEmailToNoCourse({ courseId, ...props }) {
    const [subject, setSubject] = useState("");
    const [message, setMessage] = useState("");
    const [result, setResult] = useState("");
    const { colors } = useContext(ThemeContext);
    const TEXTS = Texts.FORM_SEND_EMAIL_TO_NO_COURSE;
    const [submit, { loading }] = useMutation(SEND_EMAIL_TO_NO_COURSE);
...
export default FormSendEmailToNoCourse;